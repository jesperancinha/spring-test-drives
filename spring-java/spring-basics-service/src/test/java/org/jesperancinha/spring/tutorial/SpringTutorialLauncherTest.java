...
import org.jesperancinha.spring.tutorial.controller.TimeController;
...
class SpringTutorialLauncherTest {
    ...
    void testCurrentTimeWhenRequest_noErrors() throws Exception {
        mockMvc.perform(post("/time/time")
                .contentType("application/text")
    ...
    void thousandDollars_whenRequest_noErrors() throws Exception {
        mockMvc.perform(post("/time/dollars")
                .contentType("application/text")
                .header("dollars", "150"))
                .andExpect(status().isOk());
    }
